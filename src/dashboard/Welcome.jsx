...
const Welcome = () => {
    ...
    const { refetch: dashboard } = useQuery(
        "dashboard",
    ...
    const { mutate: resendOtp, isLoading } = useMutation(
        (type) => UserApi.resendVerificationCode(type),
        {
            onSuccess: ({ data }) => {
                if (data.status == 'error') {
                    ...
                    navigate('/otp-verification/welcome')
                }
            },
        }
    );

    const { refetch: refetchReferralLink } = useQuery(() =>
        "referral-link",
        ...
        <MasterLayout>
            <div className="row gy-lg-4 gy-md-3 gy-3 align-items-center">
                {dashboardData.status == "error" && (
                    <div className="alert col-lg-12 alert-danger mb-3" role="alert">
                        <div className="text-danger fw-bolder my-2">ALERT!!!</div>

                        {dashboardData.message.error[0]}
                        {dashboardData?.data?.email_verified === 0 && (
                            <div>
                                <Link to="#" onClick={() => resendOtp('email')}>
                    ...
                    <Link to="/dashboard/transactions" className="d-block">
                        <div className="dashboard-widget ballance">
                            <div className="dashboard-widget__content">
                                ...
                                <h3 className="dashboard-widget__number">{profile?.balance ?? 0.00}</h3>
                            </div>
                        </div>
                    </Link>
                </div>
                    ...
                    <div className="dashboard-widget refer">
                        <div className="custom-border flex-align flex-between">
                            <div className="refer__content">
                                ...
                                <h5 className="refer__link" id="ref">
                                    {referralLink}
                                </h5>
                            </div>
                            <span
                                className="refer__icon dashboard-widget__icon flex-center copy-icon copyBtn"
                        ...
                        <div className="col-xxl-6">
                            <div className="dashboard-table">
                                ...
                                <table className="table table--responsive--md">
                                    ...
                                    <tbody>
                                        {dashboardData?.latest_credits?.data?.length > 0 &&
                                            dashboardData?.latest_credits?.data.map(
                                                (credit, index) => (
                                                    <tr key={index}>
                                                        <td>{credit.id}</td>
                        ...
                        <div className="col-xxl-6">
                            <div className="dashboard-table">
                                ...
                                <table className="table table--responsive--md">
                                    ...
                                    <tbody>
                                        {dashboardData?.latest_debits?.data?.length > 0 &&
                                            dashboardData?.latest_credits?.data.map(
                                                (debits, index) => (
                                                    <tr key={index}>
                                                        <td>{debits.id}</td>
