...
const BeneficiariesWithin = () => {
  ...
  const [accountCheckMessage, setAccountCheckMessage] = useState(null);
  ...
  const { refetch } = useQuery(
    "get-own-beneficiaries",
    UserApi.getOwnBeneficiaries,
    {
      onSuccess: ({ data }) => {
  ...
  const checkAccountNumber = async (accountNumber) => {
    ...
    try {
      const { data } = await UserApi.checkAccountNumber(accountNumber);

  

      if (data.status === "success") {
        // Populate account_name field with the returned account_name
        benefitForm.setFieldValue("account_name", data?.data?.user.account_name);

        setAccountCheckMessage({
          type: "success",
          text: "Account found successfully.",
        });
      } else {
        setAccountCheckMessage({ type: "error", text: data.message.error[0] });
        // Clear account_name if check fails
        benefitForm.setFieldValue("account_name", "");
      }
    } catch (error) {
      setAccountCheckMessage({
        type: "error",
        text: "An error occurred while checking the account number.",
      });
      benefitForm.setFieldValue("account_name", "");
    } finally {
  ...
  return (
    <MasterLayout>
      <Tab />
      <div
        className={`card custom--card mb-4 ${!isModalVisible ? "d-none" : ""}`}
        id="addForm"
      >
        <div className="card-header">
          <div className="d-flex justify-content-between align-items-center">
            ...
            <button
              className="btn btn--sm btn--danger close-form"
              onClick={handleClose}
              type="button"
            >
          ...
          <form method="POST" onSubmit={benefitForm.handleSubmit}>
            <div className="form-group">
              ...
              <input
                ...
                className={`form--control ${
                  benefitForm.errors.short_name &&
                  benefitForm.touched.short_name
            ...
            <div className="form-group">
              ...
              <input
                ...
                className={`form--control ${
                  benefitForm.errors.account_name &&
                  benefitForm.touched.account_name
                    ? "border border-danger"
                    : ""
                }`}
                name="account_name"
            ...
            <div className="form-group">
              ...
              <input
                onChange={(e) => {
                  benefitForm.handleChange(e);
                  checkAccountNumber(e.target.value);
                }}
                onBlur={benefitForm.handleBlur}
                value={benefitForm.values.account_number}
                className={`form--control ${
                  benefitForm.errors.account_number &&
                  benefitForm.touched.account_number
                    ? "border border-danger"
                    : ""
                }`}
                name="account_number"
              ...
              />
              {accountCheckLoading && (
                <small className="text-info">Checking account...</small>
              )}
              {accountCheckMessage && (
                <small
                  className={
                ...
                >
                  {accountCheckMessage.text}
                </small>
              )}
            </div>

            <button
              className="btn w-100 btn--base"
              type="submit"
              disabled={isLoading}
            >
              {isLoading ? (
      ...
      <div className="card custom--card overflow-hidden">
        <div className="card-header">
          <div className="header-nav mb-0">
            ...
            >
              <i className="la la-plus-circle"></i> Add New Beneficiary
            </button>
          </div>
        </div>
            ...
            <table className="table table--responsive--md">
              ...
              <tbody>
                {beneficiaries?.beneficiaries?.data?.length > 0 &&
                  beneficiaries.beneficiaries.data.map((benefit, index) => (
                    <tr key={index}>
                      ...
                      <td>
                        <div className="button-group">
                          <button
                            className="btn btn--sm btn--primary seeDetails"
                            data-id={benefit.id}
                          >
                            <i className="la la-desktop"></i>
                          </button>
                          <button className="btn btn--sm btn--base EditBeneficiary">
